  ...
  ~TriangleMesh() {}
    ...
    bool intersect(const Ray& r, Vector& P, Vector& normal, double& t){
            ...
            for (int i = 0; i < indices.size(); i++){
                ...
                Vector AO = r.C - A;
                Vector AO_cross_u = cross(AO, r.u);
                double invU_cross_N = 1./dot(r.u, N);
                double beta = - dot(e2, AO_cross_u)*invU_cross_N;
                double gamma = dot(e1, AO_cross_u)*invU_cross_N;
                double alpha = 1 - beta - gamma;
                double localt = -dot(AO, N)*invU_cross_N;
...
int main() {
    ...
    Sphere Lumiere(scene.L, 15, Vector(1., 1., 1.));
