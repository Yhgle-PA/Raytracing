...
int main() {
    ...
    scene.I = 5E9;
    double nb_images = 100.;

    std::vector<unsigned char> image(W*H * 3, 0);
    for (int num_pos_cam = 0; num_pos_cam < nb_images; num_pos_cam++){
        Vector C(35*sin(num_pos_cam/(nb_images-1)*360*2*M_PI/360), 0, 20+35*cos(num_pos_cam/(nb_images-1)*360*2*M_PI/360));
        Vector rigth(cos(-num_pos_cam/(nb_images-1)*360*2*M_PI/360), 0, sin(-num_pos_cam/(nb_images-1)*360*2*M_PI/360));
        Vector up(0, 1, 0);
        Vector viewDirection = -cross(up, rigth);
    ...
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            ...
            for (int k=0; k<nbrays; k++){
                ...
                u = u.get_normalized();
                u = u[0]*rigth + u[1] * up + u[2]*viewDirection;
...
*/                Ray r(C, u);
    ...
    std::string filename = "image_rot_" + std::to_string(num_pos_cam) + ".png";
